...
class WordLadderPuzzle(Puzzle):
    ...
    def __eq__(self, other: WordLadderPuzzle) -> bool:
        ...
        True
        """
        x = (self.from_word == other.from_word,
    ...
    def extensions(self) -> List[WordLadderPuzzle]:
        ...
        """
        y = []
        clean_set = self.word_set.copy()

        for i in self.word_set:
            if len(i) != len(self.from_word) or not set(i) <= set(self._chars):
                clean_set.remove(i)
            else:
                x = 0
                for k in range(len(i)):
                    if self.from_word[k] == i[k]:
                        x += 1
                if x == (len(i) - 1):
                    y.append(WordLadderPuzzle(i,
                                              self.to_word,
                                              clean_set))
