...
class ExpressionTreePuzzle(Puzzle):
    ...
    def is_solved(self) -> bool:
        ...
        if x:
            x = self._tree.eval(self.variables) == self.target
    ...
    def __str__(self) -> str:
        ...
        ((a * (b + 6 + 6)) + 5) = 61
        """
        return f'{str(self.variables)}\n{self._tree.__str__()} = {self.target}'
    ...
    def extensions(self) -> List[ExpressionTreePuzzle]:
        ...
        for i in self.variables:
            if self.variables[i] == 0:
                for k in range(1, 10):
                    a = ExpressionTreePuzzle(self._tree.copy(), self.target)
                    a.variables[i] = k
                    y.append(a)
    ...
    def fail_fast(self) -> bool:
        ...
        True
        """
        dic_low = self.variables.copy()
        dic_up = self.variables
        for i in dic_low:
            if dic_low[i] == 0:
                dic_low[i] = 1
                dic_up[i] = 9

        tree_low = self._tree.copy()
        lower_bound = tree_low.eval(dic_low)

        tree_up = self._tree.copy()
        upper_bound = tree_up.eval(dic_up)

        return not lower_bound <= self.target <= upper_bound
