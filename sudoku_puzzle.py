...
class SudokuPuzzle(Puzzle):
    ...
    def fail_fast(self) -> bool:
        ...
        for r in range(0, self._n):
            for c in range(0, self._n):
                union_rcs = (self._row_set(r)
                             | self._column_set(c)
                             | self._subsquare_set(r, c))

                if self._grid[r][c] not in self._symbol_set and \
                        (union_rcs & self._symbol_set) == self._symbol_set:
                    return True
        return False
    ...
    def has_unique_solution(self) -> bool:
        ...
        x = self
        y = [self]
        while not q.is_empty() or x is not None and not x.is_solved():
