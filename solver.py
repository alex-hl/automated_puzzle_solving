...
class DfsSolver(Solver):
    ...
    def solve(self, puzzle: Puzzle,
        ...
        if puzzle.fail_fast():
            ...
            x = [puzzle]
            grids = puzzle.extensions()
            if seen is None:
                seen = set()
            seen.add(str(puzzle))
            num = len(x)
            for i in grids:
                if not i.fail_fast() and not str(i) in seen:
                    seen.add(str(i))
                    if not x[-1].is_solved():
                        x = x[0:num] + self.solve(i, seen)
                seen.add(str(i))
            if x[-1].is_solved():
                return x
            else:
    ...
    def solve(self, puzzle: Puzzle,
        ...
        while not q.is_empty() or x is not None and not x.is_solved():
            ...
            for i in x.extensions():
                if not i.fail_fast() and not str(i) in seen and i != y[0]:
                    seen.add(str(i))
                    q.enqueue(i)
                    y.append(i)
                seen.add(str(i))
            x = q.dequeue()
        if y[-1].is_solved() or (len(y) >= 2 and y[-2].is_solved()):
            if y[-1].is_solved():
                fl = [y[-1]]
                a = y[-1]
            else:
                ...
                a = y[-2]
            for i in reversed(y):
                if a in i.extensions():
                    fl.append(i)
                    a = i
            return list(reversed(fl))
        else:
            return []
