...
class ExprTree:
    ...
    def eval(self, lookup: Dict[str, int]) -> int:
        ...
        31
        >>> dic = {'a': 4, 'b': 6}
        >>> exp_t2 = ExprTree('+', [ExprTree('*',[ExprTree(3, []), \
        ExprTree('b', [])]), ExprTree('a', []) , ExprTree('+', \
        [ExprTree('*', [ExprTree(7, []), ExprTree('b', [])]), \
        ExprTree(5, [])])])
        >>> exp_t2.eval(dic)
        69
        """
        if self._subtrees == [] and isinstance(self._root, int):
            return self._root
        elif self._subtrees == [] and isinstance(self._root, str):
            return lookup[self._root]
        else:
            if self._root == OP_ADD:
                x = 0
                for subtree in self._subtrees:
                    x += subtree.eval(lookup)
                return x
            else:
                y = 1
                for subtree in self._subtrees:
                    y *= subtree.eval(lookup)
                return y
    ...
    def __str__(self) -> str:
        ...
        if self.is_empty():
            ...
            if self._root == OP_ADD:
                x = '('
                for subtree in self._subtrees:
                    x += ' ' + subtree.__str__() + ' ' + OP_ADD
                return '(' + x[2:-2] + ')'
            else:
                y = '('
                for subtree in self._subtrees:
                    y += ' ' + subtree.__str__() + ' ' + OP_MULTIPLY
                return '(' + y[2:-2] + ')'
    ...
    def populate_lookup(self, lookup: Dict[str, int]) -> None:
        ...
        if self.is_empty():
            ...
            return None
        elif not self._subtrees and isinstance(self._root, str):
